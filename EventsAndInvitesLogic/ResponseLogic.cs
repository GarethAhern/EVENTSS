    ...
    public static class ResponseLogic
    {
        ...
        public static bool CheckCSVData(DataTable csvDT, ref DataTable ResponsesListDT, int eventId)
        {
            foreach (DataRow row in csvDT.Rows)
            {
                ...
                ////Add attending and numberof places reserved to datatable
                //DataRow inviteRow = inviteListDT.NewRow();

                //inviteRow[0] = InviteId;
                //inviteRow[1] = ClientAttending;
                //if (ClientAttending)
                //{
                //    inviteRow[2] = PlacesReserved;
                //}
                //inviteListDT.Rows.Add(inviteRow);


                //Add replies to custom questions to another datatable
                //if (csvDT.Columns.Count > 3)
                //{
                //Change email address to InviteId
